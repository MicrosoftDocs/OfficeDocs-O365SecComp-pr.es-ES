...
Mientras el `ViewableByExternalUsers` (propiedad) representa el estado de si se comparte un documento o un sitio con usuarios externos, hay algunas advertencias a lo que hace y no refleja esta propiedad. En los siguientes escenarios, el valor de la `ViewableByExternalUsers` no se actualizan (propiedad), y los resultados de una consulta de búsqueda de contenido que usa esta propiedad pueden ser imprecisos. 
...
## <a name="searching-for-skype-for-business-conversations"></a>Búsqueda de Skype para conversaciones de negocios

Puede usar la siguiente consulta de palabra clave para buscar específicamente para contenido de Skype para conversaciones de negocios:

```
kind:im
```

Tenga en cuenta que la consulta de búsqueda anterior devolverá también chats de Microsoft Teams. Para evitar esto, puede restringir los resultados de búsqueda para incluir solo Skype para conversaciones de negocios mediante el uso de la siguiente consulta de palabra clave:

```
kind:im AND subject:conversation
```

La consulta de palabra clave anterior excluye chats en Microsoft Teams porque Skype para conversaciones de negocios se guardan como mensajes de correo electrónico con una línea de asunto que comienza por la palabra "Conversación".

Para buscar Skype para las conversaciones de negocio que se produjeron dentro de un intervalo de fechas específico, use la siguiente consulta de palabra clave:
