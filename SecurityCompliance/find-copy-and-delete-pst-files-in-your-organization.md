...
- **\>__ JobName buscar\<DateTimeStamp\>. log: el archivo de registro contiene los mensajes de \<** estado mostrados. Este archivo se crea en la carpeta especificada por el `LogLocation` parámetro. 
...
- **\>__ JobName Delete\<DateTimeStamp\>. log: el archivo de registro contiene los mensajes de \<** estado mostrados. Este archivo se crea en la carpeta especificada por el `LogLocation` parámetro. 
    
- **\>__ JobName Delete\<DateTimeStamp\>. xml: el archivo XML solo contiene información sobre los valores de parámetro que usa la herramienta se ejecutó en el \<** modo de eliminación. También muestra el nombre y la ruta de acceso del archivo de cada archivo PST que se eliminó. Este archivo se crea en la carpeta especificada por el `ConfigurationLocation` parámetro. 
